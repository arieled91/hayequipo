...
alter table game.game add constraint FK9i9qkunl208s2lyr6cwtnh56q foreign key (location_id) references location;
alter table game.player add constraint FKj57d4kgk8qu7i73lu7xsbq8fb foreign key (user_id) references auth."user";
alter table game.players add constraint FKihptitk4lnv75may66qnaycyd foreign key (player_id) references player;
alter table game.players add constraint FKp6fpr78wx0dw2gymua6lw4t3w foreign key (game_id) references game;
alter table auth.roles_privileges add constraint FK5yjwxw2gvfyu76j3rgqwo685u foreign key (privilege_id) references privilege;
alter table auth.roles_privileges add constraint FK9h2vewsqh8luhfq71xokh4who foreign key (role_id) references role;
alter table auth.users_roles add constraint FKt4v0rrweyk393bdgt107vdx0x foreign key (role_id) references role;
alter table auth.users_roles add constraint FKgd3iendaoyh04b95ykqise6qh foreign key (user_id) references "user";
alter table auth.verification_token add constraint FK_VERIFY_USER foreign key (user_id) references "user";
